...
**Rule:** never modify or delete an existing file under `raw/`. Files added by the user are authoritative and immutable to the agent. New files may only be *added*, only by a skill that documents this behavior, and only into `raw/tmp/`, `raw/download/`, or `raw/discovered/`. Every other path under `raw/` is read-only.
...
- `_lumina/tools/` — Python tools (always: `extract_pdf.py`, `fetch_pdf.py`, `verify_quotes.py`, `requirements.txt`; research pack adds `_env.py`, `prepare_source.py`, `init_discovery.py`, `discover.py`, and fetcher tools)
...
| Topic      | `topics/`     | Thematic cluster grouping related concepts and sources; created via `/lumi-research-topic` (research) |
| Foundation | `foundations/`| Prerequisite/background knowledge; terminal page (research)          |
| Chapter    | `chapters/`   | Per-chapter notes for a book or long-form work (reading)             |
...
| Reflection | `reflections/`| Personal understanding of a concept; rewritable + append-only log (learning) |
...
| `topics/T` writes `[[concept-K]]`           | `concepts/K` appends T to `Topics`         |
| `topics/T` writes `[[source-A]]`            | `sources/A` appends T to `Topics`          |
| `chapters/Ch` writes `[[character-X]]`      | `characters/X` appends Ch to `Key chapters`|
...
- **`foundations/**`** — terminal pages
- **`outputs/**`** — ephemeral artifacts
- **External URLs** (`*://*`) — out of wiki scope
- **`reflections/**`** — personal overlay; never requires reverse links from academic pages
...
- **`_lumina/scripts/discover-runner.mjs`** — one-shot scheduled discovery runner; collects scored candidates but does not ingest or download papers.
...
- **`_lumina/tools/_env.py`** — shared `.env` loader for research tools.
