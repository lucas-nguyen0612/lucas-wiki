...
Luôn giao tiếp với người dùng bằng **Vietnamese**. Luôn viết các trang wiki bằng **Vietnamese**.
...
- Dùng **Vietnamese** cho mọi tin nhắn hội thoại. Không trộn ngôn ngữ trừ khi trích dẫn văn bản nguồn, tên tệp, lệnh hoặc danh từ riêng.
...
**Quy tắc:** không bao giờ sửa đổi hoặc xóa tệp hiện có trong `raw/`. Các tệp do người dùng thêm vào là có thẩm quyền và bất biến với agent. Chỉ có thể *thêm* tệp mới, chỉ bởi skill ghi lại hành vi này và chỉ vào `raw/tmp/`, `raw/download/`, hoặc `raw/discovered/`. Mọi đường dẫn khác trong `raw/` là chỉ đọc.
...
- **`foundations/**`** — trang cuối cùng
- **`outputs/**`** — tạo phẩm tạm thời
- **URL bên ngoài** (`*://*`) — ngoài phạm vi wiki
- **`reflections/**`** — lớp phủ cá nhân; không yêu cầu liên kết ngược từ các trang học thuật
